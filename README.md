...
Приложение показывает на экране входа локальное видео
`wallSchedule/Resources/proxygen-guide.mov`. Без этого файла экран
показывает заглушку с подсказкой, куда его положить.

Текст, которому следует запись:
