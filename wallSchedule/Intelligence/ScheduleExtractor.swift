...
struct ExtractedLesson {
    ...
    let subject: String
    @Guide(description: "Кабинет дословно как написано на фото: число или сокращение (например «с/з» — спортзал, «акт.зал»), не выдумывай число вместо сокращения. Если у урока две группы с разными кабинетами, укажи оба через « / »")
    let room: String
}
...
enum ScheduleExtractor {
    ...
    static func extractSchedule(from image: UIImage, targetClassName: String?) async throws -> [ManualLessonEntry] {
        ...
        let commonRules = "Кабинет под названием предмета не всегда число — часто это сокращение (с/з — спортзал, акт.зал — актовый зал и т.п.), переписывай его точно как на фото. Если у урока указаны две группы с двумя учителями и двумя кабинетами (обычно английский или физкультура, формат «Учитель1:1 Учитель2:2» с кабинетами через «/»), укажи оба кабинета через « / », не выбирай один наугад."
